 ...
 * <p>
 * This class presents a modal dialog that displays available powerup types, their costs, and effects. The shop ensures that players have
 * sufficient coins before allowing a purchase.
 *
 * @author feljo718
 * @see PowerupType
 * @see Tower
...
{
    private static final int POWERUP_DURATION = 30000;
	...
	// Add a header
	JLabel headerLabel = new JLabel("Available Powerups");
	headerLabel.setFont(headerLabel.getFont().deriveFont(Font.BOLD, HEADER_FONT_SIZE));
	headerLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
	headerLabel.setBorder(BorderFactory.createEmptyBorder(PADDING, PADDING, PADDING, PADDING));
	dialog.add(headerLabel);

	JPanel buttonPanel = new JPanel();
	buttonPanel.setLayout(new GridLayout(PowerupType.values().length, 1, GRID_HGAP, GRID_VGAP));
	...
	JButton button = new JButton(String.format("%s (%d coins) - %s", type.getName(), type.getCost(), type.getDescription()));
	button.addActionListener(e -> handlePowerupPurchase(type));
	return button;
    }

    /**
     * Handles the powerup purchase logic when a powerup button is clicked.
     *
     * @param type The powerup type to purchase
     */
    private void handlePowerupPurchase(PowerupType type) {
	if (board.getCoins() >= type.getCost()) {
	    // Check if the powerup is already active
	    if (targetTower.hasActivePowerupOfType(type)) {
		JOptionPane.showMessageDialog(dialog, "This powerup is already active on the tower!", "Powerup Active",
					      JOptionPane.INFORMATION_MESSAGE);
		return;
	    }

	    int result = JOptionPane.showConfirmDialog(dialog, "Apply " + type.getName() + " to this tower?", "Confirm Purchase",
						       JOptionPane.YES_NO_OPTION);

	    if (result == JOptionPane.YES_OPTION) {
		board.gainCoins(-type.getCost());
		targetTower.applyPowerup(type, POWERUP_DURATION);
